...
namespace CoreERP.Controllers
{
    ...
    public class CommonController : ControllerBase
    {
        ...
        [HttpGet("GetSaleOrderData/{CompanyCode}")]
        public async Task<IActionResult> GetSaleOrderData(string CompanyCode)
        {
            var result = await Task.Run(() =>
            {
                try
        ...
        public async Task<IActionResult> GetMaterialList()
        {
            var result = await Task.Run(() =>
            {
                ...
                {
                    dynamic expando = new ExpandoObject();
                    expando.materialList = _materialMasterRepository.GetAll().Select(x => new { ID = x.MaterialCode, TEXT = x.Description, AvailQTY = x.ClosingQty, Rate = x.ClosingPrice, netWeight = x.NetWeight, Hsnsac = x.Hsnsac, BOM = x.BOM });
                    return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> GetMaterialList(string CompanyCode, string MaterialCode)
        {
            var result = await Task.Run(() =>
            {
                try
                {
                    var mmasterList = _materialMasterRepository.GetAll().Where(c => c.Company.Contains(CompanyCode) && c.MaterialCode.Contains(MaterialCode)).Select(x => new { ID = x.MaterialCode, TEXT = x.Description, AvailQTY = x.ClosingQty, Rate = x.ClosingPrice, netWeight = x.NetWeight, grossWeight = x.GrossWeight, Hsnsac = x.Hsnsac, CustomerCode = x.CustomerCode, BOM = x.BOM });
                    if (mmasterList.Any())
                    {
                        dynamic expdoObj = new ExpandoObject();
        ...
        public async Task<IActionResult> GetCustomerList(string CompanyCode)
        {
            var result = await Task.Run(() =>
            {
                try
                {
                    var bpList = CommonHelper.BPList().Where(y => y.Company == CompanyCode).Select(x => new { ID = x.Bpnumber, TEXT = x.Name, BPTYPE = x.BpTypeName, BPGROUP = x.BpGroupName, GstNo = x.Gstno });
                    if (bpList.Any())
                    {
                        dynamic expdoObj = new ExpandoObject();
        ...
        public async Task<IActionResult> GetPurchaseInvoiceList()
        {
            var result = await Task.Run(() =>
            {
                try
                {
                    var purchaseinvoiceList = _InvoiceMemoHeaderRepository.GetAll().Where(z => z.Status == "N").Select(x => new { x.PartyAccount, x.PartyInvoiceNo, x.VoucherNumber, x.TotalAmount, x.PostingDate, x.Paymentterms, x.DueDate, x.BalanceDue, x.ClearedAmount,x.ReferenceNumber });
