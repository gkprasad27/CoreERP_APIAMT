    ...
    public class CommonController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetMaterialList()
        {
            var result = await Task.Run(() =>
            {
                ...
                {
                    dynamic expando = new ExpandoObject();
                    expando.materialList = _materialMasterRepository.GetAll().Select(x => new { ID = x.MaterialCode, TEXT = x.Description, AvailQTY = x.ClosingQty, Rate = x.ClosingPrice, netWeight = x.NetWeight, Hsnsac = x.Hsnsac });
                    return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> GetPurchaseInvoiceList()
        {
            var result = await Task.Run(() =>
            {
                try
                {
                    var purchaseinvoiceList = _InvoiceMemoHeaderRepository.GetAll().Select(x => new { x.PartyAccount, x.PartyInvoiceNo, x.TotalAmount, x.PostingDate, x.Paymentterms, x.DueDate,x.BalanceDue,x.ClearedAmount});
