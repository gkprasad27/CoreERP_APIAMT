...
namespace CoreERP.Controllers
{
    ...
    public class BrandModelController : ControllerBase
    {
        ...
        
        {
            ...
            try
            {
                if (BrandModelHelpers.GetList(brandModel.BrandName).Count > 0)
                    return Ok(new APIResponse() { status = APIStatus.FAIL.ToString(), response = $" code={brandModel.BrandName} Already exists." });
        ...
        public async Task<IActionResult> GetMaterialGroups()
        {
            ...
            {
                dynamic expando = new ExpandoObject();
                expando.MaterialGroupsList=MaterialGroupHelper.GetMaterialGroupList().Select(mtgrp => new { ID = mtgrp.GroupCode, TEXT = mtgrp.GroupName });
                return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetCompanies()
        {
            ...
            {
                    dynamic expando = new ExpandoObject();
                    expando.CompaniesList = BrandModelHelpers.GetCompanies().Select(comp=> new { ID=comp.CompanyId,TEXT=comp.CompanyName});
                    return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetInputTaxes()
        {
            ...
            {
                dynamic expando = new ExpandoObject();
                expando.InputTaxesList = TaxmasterHelper.GetListOfTaxMasters().Select(s => new { ID = s.Code, TEXT = s.Description });
                return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetOutPutTaxes()
        {
            ...
            {
                dynamic expando = new ExpandoObject();
                expando.OutPutTaxesList = TaxmasterHelper.GetListOfTaxMasters().Select(s => new { ID = s.Code, TEXT = s.Description });
                return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
