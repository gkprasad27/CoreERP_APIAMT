...
namespace CoreERP.Controllers.Selfservice
{
  ...
  public class PermissionRequestController : ControllerBase
  {
        ...
        [HttpGet("GetPermissionApplDetailsList/{code}")]
        public IActionResult GetPermissionApplDetailsList(string code)
        {
            try
            {
                dynamic expando = new ExpandoObject();
                expando.PermissionApplDetailsList = PermissionRequestHelper.GetPermissionApplDetailsList(code).ToList();
                return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
        ...
        [HttpPut("UpdatePermissionapplying")]
        public IActionResult UpdatePermissionapplying([FromBody]PermissionRequest permissionRequest)
        {
            if (permissionRequest == null)
                return Ok(new APIResponse() { status = APIStatus.FAIL.ToString(), response = "Requst can not be empty." });
            try
            {
                ...
                PermissionRequest result = PermissionRequestHelper.UpdatePermissionapplying(permissionRequest, out errorMessge);
                if (result != null)
                    return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = result });
