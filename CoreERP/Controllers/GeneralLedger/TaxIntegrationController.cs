...
namespace CoreERP.Controllers
{
    ...
    public class TaxIntegrationController : ControllerBase
    {
        ...
        public IActionResult GetCompanysList()
        {
            ...
            {
                dynamic expando = new ExpandoObject();
                expando.CompanysList = GLHelper.GetCompanies().Select(x => new { ID = x.CompanyCode, TEXT = x.CompanyName });
                return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetGLTaxAccountList()
        {
            ...
            {
                dynamic expando = new ExpandoObject();
                expando.GLTaxAccountList = GLHelper.GetTaxAccounts().Select(x => new { ID = x.AccountNumber, TEXT = x.GlaccountName });
                return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
