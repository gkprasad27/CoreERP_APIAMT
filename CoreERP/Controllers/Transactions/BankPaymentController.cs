    ...
    public class BankPaymentController : Controller
    {
        ...
        public async Task<IActionResult> GetVoucherNo(string branchCode)
        {
            var result = await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    dynamic expando = new ExpandoObject();
                    expando.BranchesList = new BankPaymentHelper().GetVoucherNo(branchCode, out errorMessage);
                    return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> GetAccountLedgerList(string ledegerCode)
        {
            var result = await Task.Run(() =>
            {
                ...
                {
                    dynamic expando = new ExpandoObject();
                    expando.AccountLedgerList = new BankPaymentHelper().GetAccountLedgers(ledegerCode).OrderBy(acl => acl.LedgerCode.Length).Select(x => new { ID = x.LedgerCode, TEXT = x.LedgerName });
                    return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> GetAccountLedgerListByName(string ledegerName)
        {
            var result = await Task.Run(() =>
            {
                ...
                {
                    dynamic expando = new ExpandoObject();
                    expando.AccountLedgerList = BankPaymentHelper.GetAccountLedgerByName(ledegerName).Select(x => new { ID = x.LedgerCode, TEXT = x.LedgerName });
                    return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
                }
                catch (Exception ex)
                {
