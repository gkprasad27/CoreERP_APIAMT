    ...
    public class CashPaymentController : Controller
    {
        ...
        public async Task<IActionResult> GetBranchesList()
        {
            try
            {
                ...
                expando.BranchesList = CashPaymentHelper.GetBranchesList().Select(x => new { ID = x.BranchCode, TEXT = x.Name });
        ...
        public async Task<IActionResult> GetVoucherNo(string branchCode)
        {
            ...
            {
                dynamic expando = new ExpandoObject();
                expando.BranchesList = CashPaymentHelper.GetVoucherNo(branchCode);
                return Ok(new APIResponse() { status = APIStatus.PASS.ToString(), response = expando });
            }
            catch (Exception ex)
            {
